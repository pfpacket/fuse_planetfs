...
#define MODULE_NAME "net_dns_mod"
...
extern "C" {
    ...
    {
        ::syslog(LOG_NOTICE, "module=%s: %s: called", __FILE__, __PRETTY_FUNCTION__);
    ...
    {
        ::syslog(LOG_NOTICE, "module=%s: %s: path=%s", __FILE__, __PRETTY_FUNCTION__, path.string().c_str());
        return resolver->open(file_ent, path);
    }

    int planet_mod_read(shared_ptr<fs_entry> file_ent, char *buf, size_t size, off_t offset)
    {
        ::syslog(LOG_NOTICE, "module=%s: %s: size=%d, offset=%llu", __FILE__, __PRETTY_FUNCTION__, size, offset);
